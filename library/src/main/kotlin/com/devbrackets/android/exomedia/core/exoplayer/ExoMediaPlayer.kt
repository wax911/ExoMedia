...
class ExoMediaPlayer(private val context: Context) : Player.EventListener {
    ...
    val windowInfo: WindowInfo?
        get() {
            ...
            val currentWindow = timeline.getWindow(currentWindowIndex, Timeline.Window(), true)
    ...
    fun setUri(uri: Uri?) {
        setMediaSource(if (uri != null) ExoMedia.Data.mediaSourceProvider.generate(context, mainHandler, uri, bandwidthMeter) else null)
    }
...
// In the current implementation only one track can be selected at a time so get the first one.
    ...
    private class StateStore {
        ...
        companion object {
            val FLAG_PLAY_WHEN_READY = -0x10000000
    ...
    private inner class ComponentListener : VideoRendererEventListener, AudioRendererEventListener, TextOutput, MetadataOutput {
    ...
    companion object {
        ...
        private val BUFFER_REPEAT_DELAY = 1000
        private val WAKE_LOCK_TIMEOUT = 1000
    }
}